...
class PolymorphicRepository{
    ...
    async getUniqueId(polymorphicInfo: PolymorphicInfo): Promise<number>{
        const [rows]: any[] = await this.connector.query('SELECT unique_id FROM message_polymorphics WHERE polymorphic_table = ? AND polymorphic_id = ? ', [polymorphicInfo.polymorphic_table,polymorphicInfo.polymorphic_id]);
        if(rows.length > 0){
    ...
    async getUniqueIdByMessageId(message_id: string): Promise<number | null>{
        const [rows]: any[] = await this.connector.query('SELECT unique_id FROM message_polymorphics WHERE message_id = ? ', [message_id]);
        if(rows.length > 0){
            return rows[0].unique_id;
        }
        return null;
